...
import { generateLightColorHex, YEAR_INITIAL_STATE, MONTH_INITIAL_STATE, monthOptions, yearOptions, } from 'utils/statisticsSetting';
...
// /api/transactions-summary"
  ...
  useEffect(() => {
    async function getStats() {
      try {
        const res = await axios.get(`/api/transactions-summary?month=${selectMonth}&year=${selectYear}`);
        return res.data;
      } catch (error) {
        console.log(error);
      }
...
// console.log(obj?.categoriesSummary.length)
  ...
  return (<>
    ...
    <Selector options={monthOptions} initialState={MONTH_INITIAL_STATE} changeSelector={onSelectMonth} />
    <Selector options={yearOptions} initialState={YEAR_INITIAL_STATE} changeSelector={onSelectYear}/>
    {obj?.categoriesSummary.length === 0 ?
      (<h2>you have no transactions this month</h2>) 
      : (<div>
    <ul>
      {obj?.categoriesSummary
      .map(({ name, total, type }) => (
